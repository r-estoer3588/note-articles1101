...
class SNSIntegratedAnalyzer:
    ...
    def __init__(self, data_dir: str = "outputs/sns_tracking"):
        self.data_dir = Path(data_dir)
        self.data_dir.mkdir(parents=True, exist_ok=True)
        self.data_file = self.data_dir / "sns_data.json"
    ...
    def manual_update(self):
        ...
        note_sales = input(f"{Color.CYAN}note 有料記事販売数（累計）: {Color.END}").strip()
        if note_sales:
            record["note_total_sales"] = int(note_sales)
        
        note_revenue = input(f"{Color.CYAN}note 売上（累計・円）: {Color.END}").strip()
        if note_revenue:
            record["note_total_revenue"] = int(note_revenue)
        
        magazine_subs = input(f"{Color.CYAN}note マガジン会員数: {Color.END}").strip()
        if magazine_subs:
            record["magazine_subscribers"] = int(magazine_subs)
        
        # Threads
        threads_followers = input(f"{Color.CYAN}Threads フォロワー数: {Color.END}").strip()
        if threads_followers:
            record["threads_followers"] = int(threads_followers)
        
        threads_posts = input(f"{Color.CYAN}Threads 今日の投稿数: {Color.END}").strip()
        if threads_posts:
        ...
        # その他収益
        other_revenue = input(f"{Color.CYAN}その他収益（ココナラ等・円）: {Color.END}").strip()
        if other_revenue:
            record["other_revenue"] = int(other_revenue)
        
        # メモ
        memo = input(f"{Color.CYAN}メモ（今日の気づき・反省など）: {Color.END}").strip()
        if memo:
            record["memo"] = memo
    ...
    def show_latest_progress(self):
        ...
        if "note_total_sales" in latest:
            ...
            print(f"note 販売数: {latest['note_total_sales']} / {goals_1m['note_sales']} ({note_progress:.1f}%)")
        
        if "note_total_revenue" in latest:
            revenue_progress = (latest["note_total_revenue"] / goals_1m["note_revenue"]) * 100
            print(f"note 売上: ¥{latest['note_total_revenue']:,} / ¥{goals_1m['note_revenue']:,} ({revenue_progress:.1f}%)")
        
        # Threads
        if "threads_followers" in latest:
            threads_progress = (latest["threads_followers"] / goals_1m["threads_followers"]) * 100
            print(f"Threads フォロワー: {latest['threads_followers']} / {goals_1m['threads_followers']} ({threads_progress:.1f}%)")
    ...
    def generate_weekly_report(self):
        ...
        if "note_total_revenue" in first and "note_total_revenue" in latest:
            revenue_growth = latest["note_total_revenue"] - first["note_total_revenue"]
            print(f"note 売上: +¥{revenue_growth:,}")
        
        if "threads_followers" in first and "threads_followers" in latest:
            threads_growth = latest["threads_followers"] - first["threads_followers"]
    ...
    def generate_monthly_report(self):
        ...
        if "note_total_sales" in latest:
            ...
            print(f"{status} note 販売数: {latest['note_total_sales']} / {goals['note_sales']} ({sales_achievement:.1f}%)")
        
        if "note_total_revenue" in latest:
            revenue_achievement = (latest["note_total_revenue"] / goals["note_revenue"]) * 100
            status = "✅" if revenue_achievement >= 100 else "🔄"
            print(f"{status} note 売上: ¥{latest['note_total_revenue']:,} / ¥{goals['note_revenue']:,} ({revenue_achievement:.1f}%)")
        
        if "threads_followers" in latest:
            threads_achievement = (latest["threads_followers"] / goals["threads_followers"]) * 100
            status = "✅" if threads_achievement >= 100 else "🔄"
            print(f"{status} Threads フォロワー: {latest['threads_followers']} / {goals['threads_followers']} ({threads_achievement:.1f}%)")
    ...
    def export_csv(self):
        ...
        csv_file = self.data_dir / f"sns_data_{datetime.now().strftime('%Y%m%d')}.csv"
